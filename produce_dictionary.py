...
produce_Frame = pd.DataFrame(produce_dictionary)
produce_Frame.index = ['Cost Per Pound','Quantity Sold','Total Sale']

print("1. Produce that had the highest and lowest sales in total sales (both name of produce and value)")
print("Highest sales in total sales")
print(produce_Frame.T.sort_values(by='Total Sale',ascending=False).iloc[0,[2]])
print("Lowest sales in total sales")
print(produce_Frame.T.sort_values(by='Total Sale',ascending=True).iloc[0,[2]])

print()

print("2. Using 'loc', display the quantity and total sales for 'Orange' and 'Beets' (together)")
print(produce_Frame.loc[['Quantity Sold','Total Sale'],['Orange','Beets']])

print()

print("3. Using 'loc', display the total sales for 'Apples' through Lettuce")
print(produce_Frame.loc['Total Sale', ['Apples','Lettuce']])
print()

print("4. Using 'at', update the quantity sold for Apricots to 11,955 and total sales to 44,353.05")
produce_Frame.at['Quantity Sold','Apricots'] = 11955
produce_Frame.at['Total Sale','Apricots'] = 44353.05
print(produce_Frame.Apricots)
print()

print("5. What is the average quantity sold across all products? (print out ONLY quantity sold)")
print(produce_Frame.loc['Total Sale'].mean())
print()

print("6. Create a new dataframe for only those produce that have sold between 11,500 to 12,000 (quantity)")
new_produce_Frame = produce_Frame.T[produce_Frame.T['Quantity Sold'] >=11500]
new_produce_Frame = produce_Frame.T[produce_Frame.T['Quantity Sold'] <= 12000]
print(new_produce_Frame)

print()

print("7. What is the total sales for the products in the above new dataframe? (print out ONLY total sales)")
print(new_produce_Frame['Total Sale'].sum())